...
const upload = multer({
  storage: multer.diskStorage({
    destination: (req, file, done) => {
      done(null, "./public/images") //그냥 /imaages나 /public/images 모두 경로를 찾아오지 못함. ./를 붙여서 localhost 주소를 받아오게 해야함.
      //여기서 ./가 바로 http://localhost:3000/를 의미함. 그렇다고 http://localhost:3000/을 써도 안됨. 파일경로를 찾아감.
      // ./로 써야만 웹접속경로를 찾아감.
    },
...
//list task
router.get('/', function (req, res, next) { //list.json이 이런 식으로 구성되면 어떻게 
  //받아야 list가 제대로 출력될 수 있을까?
  let sort = '';
  ...
  let word = '%' + req.query.word + '%'; //where는 1개만 쓰고 그 뒤에 나오는 것들은 or로 엮어야 search목적
...
router.get('/code', (req, res) => {
  ...
  db.get().query(sql, (err, rows) => {
    res.send(rows); //rows[0]으로 쓰면 JSONObject가 되고, rows로 쓰면 JSONArray가 됨. retrofit도 list 안붙이고, 붙이고의 차이가 생김.
  })
});
